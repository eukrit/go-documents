...
> **Rule 14a — Exclusive hosting.** All generated non-website HTML for this project (dashboards, reports, summaries, forms, documents, hub, build-summary, architecture) is served exclusively at `https://gateway.goco.bz/go-documents/<path>`. Do not link, share, or reference raw `*.run.app`, `storage.googleapis.com`, `raw.githubusercontent.com`, or `eukrit.github.io` URLs anywhere a reader will see (BUILD_LOGs, hub configs, READMEs, chat).
>
> **Rule 14b — Project root = hub.** `https://gateway.goco.bz/go-documents/` (and `https://gateway.goco.bz/go-documents` with no slash) resolves to this project's `docs/hub.html` for every backend kind. The slug catchall in `go-access-gateway/services/access_gateway/routes/pages.py` normalizes the empty path. Keep `docs/hub.html` fresh per Rule 13f — `verify.sh` blocks any push that leaves it stale.
>
> **Canonical paths** (must always work, mirroring `gateway.goco.bz/directory`):
> - `https://gateway.goco.bz/go-documents/docs/hub.html` — Hub
> - `https://gateway.goco.bz/go-documents/docs/build-summary.html` — Build Summary
> - `https://gateway.goco.bz/go-documents/docs/architecture.html` — Architecture
> - `https://gateway.goco.bz/go-documents/BUILD_LOG.md` — Build Log

All HTML pages, dashboards, summaries, and forms in this project are served via the **`go-access-gateway`** at `https://gateway.goco.bz/go-documents/...` — NOT directly from this project's Cloud Run URL.

- **Public URL pattern:** `https://gateway.goco.bz/go-documents/<path>`
...
- **Hub link target:** `hub.config.json` `LIVE_URL_BASE` should be `https://gateway.goco.bz/go-documents`.
...
|---|---|---|
| `ai-agents-go-<SA-KEY-FILENAME>.json` | Credentials folder | GCP service account key |
