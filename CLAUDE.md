...
All HTML pages, dashboards, summaries, and forms in this project are served via the **`go-access-gateway`** at `https://gateway.goco.bz/p/go-documents/...` — NOT directly from this project's Cloud Run URL.

- **Public URL pattern:** `https://gateway.goco.bz/p/go-documents/<path>`
...
- **Hub link target:** `hub.config.json` `LIVE_URL_BASE` should be `https://gateway.goco.bz/p/go-documents`.
